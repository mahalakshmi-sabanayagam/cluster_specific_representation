...
def real_data_loader(dataset='mnist', class_labels_list=[0, 1, 2, 3, 4], num_samples=50, normalise_data=False,
                     subsample=True, CNN=False, noise=0):
    """
...
#             X_full = X_full[:, :] / x_norm
    ...
    if noise!=0:
        ...
        return X, Y, X_noise, len(class_labels_list)
        
    return X, Y, None, len(class_labels_list)
...
def image_data_loader(dataset="mnist", path = './data', batch_size = 50, class_labels_list = [0,1,9], num_samples = 200, transform=None):
    ...
    if dataset == "mnist":
        train_dataset = datasets.MNIST(path, transform=transform, download=True)
        test_dataset  = datasets.MNIST(path, transform=transform, download=True, train=False)
    elif dataset == "fmnist":
        train_dataset = datasets.FashionMNIST(path, transform=transform, download=True)
        test_dataset  = datasets.FashionMNIST(path, transform=transform, download=True, train=False)

    # create train and test dataloaders
    train_indices = torch.tensor([])
    test_indices = torch.tensor([])
    for i in class_labels_list:
        train_indices = torch.cat((train_indices, (train_dataset.targets == i).nonzero(as_tuple=True)[0][:num_samples]))
        test_indices = torch.cat((test_indices, (test_dataset.targets == i).nonzero(as_tuple=True)[0][:int(num_samples/2)]))
    print('Number of total train and test samples ', train_indices.shape, test_indices.shape)

    train_dataset_subset = Subset(train_dataset, train_indices.long())
    test_dataset_subset = Subset(test_dataset, test_indices.long())

    train_loader = DataLoader(train_dataset_subset, shuffle=True, batch_size=batch_size)
    test_loader = DataLoader(test_dataset_subset, shuffle=False, batch_size=batch_size)
